...
public class MemberDAOImpl implements MemberDAO {
	...
	private MemberDAOImpl() {
		// Resoutrce Factory에 담겨져있는 Connection을 하나씩 Rent해오는 로직을 작성
		// Naming Service를 위한 Context 객체부터 생성..
		try {
	...
	public MemberVO login(String id, String pass) throws SQLException {
		...
		try {
			...
			if (rs.next()) {
				vo = new MemberVO(id, pass, rs.getString("name"), rs.getString("address"));
			}
		} finally {
			closeAll(rs, ps, conn);
		}
	...
	public void update(MemberVO vo) throws SQLException {
		...
		try {
			...
			System.out.println(ps.executeUpdate() + "행 업데이트 완료");
		}finally {
			closeAll(ps, conn);
		}
	}
